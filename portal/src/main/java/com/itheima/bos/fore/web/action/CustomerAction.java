...
public class CustomerAction extends ActionSupport
    ...
    public String regist() {
        ...
        if (StringUtils.isNotEmpty(checkcode)
            ...
            redisTemplate.opsForValue().set(model.getTelephone(), activeCode, 1,
                    TimeUnit.DAYS);
            String emailBody =
                    "感谢您注册本网站的帐号，请在24小时之内点击<a href='http://localhost:8080/portal/customerAction_active.action?activeCode="
                            + activeCode + "&telephone=" + model.getTelephone()
                            + "'>本链接</a>激活您的帐号";
            // 发送激活邮件
            MailUtils.sendMail("激活邮件", emailBody, model.getEmail());
            
            System.out.println(model.getEmail()+"66"+model.getTelephone());
            System.out.println(emailBody+"666");
            return SUCCESS;
        }
        return ERROR;
    }
    ...
    // 使用属性驱动获取激活码
    private String activeCode;//(模型驱动无法封装)
